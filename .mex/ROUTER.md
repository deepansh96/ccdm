...
- The Usage Stats Poster reports a configured DeepSeek home from its DeepSeek setup marker and private `api-key`: it validates the documented user-balance schema (USD/CNY only, every amount required and preserved exactly), fetches one account-wide balance per distinct key, and pairs it with this machine's local DeepSeek token totals, showing the account-wide balance and paid/granted breakdown as display-only notes with no quota card.
