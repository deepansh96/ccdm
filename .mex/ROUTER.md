...
- The foreground Conversation Reminder service observes registered project channels with root credentials, consumes both provider adapters' durable events, and persists owner closure, acknowledgment, reopening, and due times in a private SQLite store. Its assigned-bot emoji delivery, cleanup, failure handling, and bounded operator recovery of uncertain sends and pending side effects are implemented; operator-requested `discover` runs bounded, checkpointed history discovery inside the foreground worker, buffering live events until each channel's baseline commits; public delivery stays gated until restart reconciliation and catch-up are available. Registry changes retire obsolete assignment generations before further sends; retired cleanup uses only the retired bot and reports inaccessible leftovers; lost access or ambiguous assignments suspend a channel; `assignment-changed` issues a fresh generation for registration workflows.
