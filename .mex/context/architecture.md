...
- **Claude reminder adapter** - opt-in `claude-reminder-channel.js` relays the official Discord plugin through a launch-scoped MCP filter; command hooks and the shared durable receiver record observe-only reply and lifecycle events. Reminder delivery remains disabled.
