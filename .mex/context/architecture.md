...
- **Claude reminder adapter** - opt-in `claude-reminder-channel.js` relays the official Discord plugin through a launch-scoped MCP filter; command hooks and the shared durable receiver record reply and lifecycle events. The foreground service owns reminder delivery, which remains gated on discovery and restart reconciliation.
