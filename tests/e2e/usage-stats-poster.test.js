...
test("poster reports a DeepSeek balance and local-session coverage without quota cards", async () => {
  ...
  assert.match(codexField.value, /\*\*deepseek-flash\*\* \(DeepSeek\)/);
  assert.match(codexField.value, /Account-wide balance: \*\*12\.34 USD\*\* \(paid 12\.34 · granted 0\.00\)/);
  assert.match(codexField.value, /Coverage: local Codex sessions only; the API exposes no account spend or quota/);
  ...
  assert.match(codexField.value, /Local Codex \d{4}-\d{2}: 1\.3k tokens · 1 session/);
...
test("DeepSeek coverage reports unavailable without hiding the real balance", async () => {
  ...
  assert.match(
    codexField.value,
    /Local usage: \*unavailable \(Local DeepSeek usage collector unavailable\)\*/,
  );
  assert.match(codexField.value, /Coverage: local Codex sessions only; the API exposes no account spend or quota/);
  assert.doesNotMatch(codexField.value, /Local Codex \d{4}-\d{2}:/);
});
...
test("DeepSeek key grouping issues one balance request per distinct key", async () => {
  ...
  assert.match(
    codexField.value,
    /\*\*deepseek-mirror \+ deepseek-primary \(shared key\)\*\* \(DeepSeek\)\nAccount-wide balance: \*\*12\.34 USD\*\* \(paid 12\.34 · granted 0\.00\)\nLocal Codex \d{4}-\d{2}: 1\.3k tokens · 1 session\n/,
  );
});
...
test("malformed or redirecting DeepSeek balance responses stay unavailable and unexposed", async () => {
  ...
  for (const testCase of cases) {
    ...
    assert.match(codexField.value, /Account-wide balance: \*unavailable/, testCase.name);
...
test("DeepSeek metric stays a codex display-only note and never shadows ordinary Codex limits", () => {
  ...
  assert.match(probe.stdout, /^first_line Account-wide balance 3\.50 USD$/m);
});
...
test("DeepSeek multi-currency balances stay separate and never sum", () => {
  ...
  assert.equal(probe.status, 0, probe.stderr || probe.stdout);
  assert.match(probe.stdout, /^line Account-wide balance 12\.34 USD · 88\.00 CNY · insufficient for API calls$/m);
  assert.doesNotMatch(probe.stdout, /100\.34/, "currencies must never be summed");
  assert.match(probe.stdout, /^text Account-wide balance: \*\*12\.34 USD\*\* \(paid 12\.34 · granted 0\.00\) · \*\*88\.00 CNY\*\* \(paid 80\.00 · granted 8\.00\) · \*insufficient for API calls\*$/m);
  assert.match(probe.stdout, /^note_count 3$/m);
...
test("legacy get_codex_stats reports real DeepSeek balance instead of a placeholder", () => {
  const probe = probePoster([
    ...
    "print('balance', 'Account-wide balance: **5.00 USD** (paid 5.00 · granted 0.00)' in text)",
    "print('local_label', 'Local usage' in text or 'Local Codex' in text)",
    "home_text = poster.get_codex_home_stats(home, 'deepseek-flash')",
    "print('home_placeholder', 'metrics path' in home_text)",
    "print('home_balance', 'Account-wide balance: **5.00 USD**' in home_text)",
