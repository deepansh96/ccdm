...
| Discord MCP | Reply/edit/react/fetch tools | Covered | Covers text replies with references, empty-text file replies, missing files, edit, react, fetch limit capping, bad negative limits, rate-limit retry, and private transcripts for paginated reads above 100 messages. |
...
| Usage stats poster | Secret-free LaunchAgent rendering | Covered | Installer scenarios validate the Python 3 prerequisite and a Pillow-free install, hand-author absolute Python, Codex, poster, and log paths plus the 600-second default, and assert no credentials, channel IDs, or config values are rendered. |
...
| Conversation reminders | Provider adapters | Covered | `conversation-reminder-claude.test.js` and `conversation-reminder-codex.test.js` drive the launch-scoped Claude channel and hooks, and the Codex bridge with scoped MCP receipts. They cover completion, progress, input-needed, `/close` interception, resumed work, outbox replay, and readiness. |
| Conversation reminders | Conversation state and delivery | Covered | `conversation-reminder-service.test.js` drives the foreground worker against the stateful Discord fake and a controllable clock. It covers the first hour, replacement ordering, reply and close races, 429s, access loss, lost sends, recovery, and store integrity. |
| Conversation reminders | History discovery | Covered | `conversation-reminder-discovery.test.js` covers bounded, checkpointed, fair history scans, eligibility, reaction ambiguity, and denied history. |
| Conversation reminders | Restart reconciliation and catch-up | Covered | `conversation-reminder-restart.test.js` covers restart, reconnect, and re-enable reconciliation, and globally spaced catch-up without replaying missed intervals. |
| Conversation reminders | Assignment retirement | Covered | `conversation-reminder-assignment.test.js` covers reassignment, deregistration, in-flight cancellation, retired-bot cleanup limits, and fresh generations. |
| Conversation reminders | Supervised LaunchAgent installation | Covered | `conversation-reminder-launchagent.test.js` asserts an exact, secret-free plist and a private state directory with private logs. It covers idempotent reinstall, failed-load rollback, refusal before any `launchctl` call for missing prerequisites, adapters, credentials, owner, or a usable store, and an unchanged Usage Stats Poster. |
| Conversation reminders | Supervised and foreground single worker | Covered | Launches the rendered `ProgramArguments` as launchd would. One worker holds the lock in either order. `disable` exits successfully so KeepAlive does not relaunch, a disabled relaunch neither observes nor sends, and re-enable reports how to restart. |
| Conversation reminders | Both-provider supervised workflow | Covered | Claude and Codex each answer through their real adapters, then the coding agents stop. The supervised worker reconciles and sends one `👀` per channel from each assigned bot. The owner's reply and `/close` clear them without a coding turn, and a supervised restart preserves the closure. |
| Conversation reminders | Live Discord, provider, and launchd execution | Deferred | Real Discord, Claude, Codex, and launchd behavior remain opt-in Live Smoke Suite checks behind the Live Gate. |
