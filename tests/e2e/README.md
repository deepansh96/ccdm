...
- `scripts/resolve-codex-home.py` validates only the top-level selector and the target project's legacy `codex_home`, preserving project-over-global-`~/.codex` precedence. It expands `~`, normalizes path components without resolving symlinks, and requires a usable home plus any present `config.toml` before startup mutates MCP config, tmux, or registry PID state.
